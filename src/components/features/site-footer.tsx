...
import { CATEGORY_SEO, categoryPath } from '@/lib/seo'
...
export async function SiteFooter() {
  ...
  // Interlinking SEO: las landings /servicios/* quedan enlazadas desde todas
  // las páginas que llevan footer.
  const serviceLinks = CATEGORY_KEYS.map((category) => ({
    href: categoryPath(category),
    label: CATEGORY_SEO[category].plural,
  }))
  ...
  return (
    <footer className="motion-footer mt-app-xl bg-surface-container border-t border-outline-variant">
      <div className="w-full max-w-7xl mx-auto px-margin-mobile md:px-margin-desktop pt-12 pb-28 md:py-14">
        <div className="grid grid-cols-1 gap-10 md:grid-cols-[minmax(0,1.2fr)_minmax(0,0.8fr)_minmax(0,0.8fr)_minmax(0,0.95fr)] md:gap-12">
          ...
          <FooterLinkGroup title={t('services')} links={serviceLinks} />
          <FooterLinkGroup
            title={t('support')}
