...
async function main() {
  ...
  await prisma.categoryConfig.createMany({
    data: [
      ...
      { category: 'ELECTRICAL', icon: 'electrical_services' },
      { category: 'MASONRY', icon: 'foundation' },
      { category: 'WELDING', icon: 'local_fire_department' },
      { category: 'ELECTRONICS', icon: 'memory' },
      { category: 'APPLIANCE_REPAIR', icon: 'home_repair_service' },
